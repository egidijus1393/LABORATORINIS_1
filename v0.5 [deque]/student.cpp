...
#include <iostream>
using namespace std;
deque<Studentas> generuoti_studentas( const unsigned n ) {
    deque<Studentas> studentas;
    Studentas s;
auto start = high_resolution_clock::now();
    for ( deque<Studentas>::size_type i = 0; n != i; ++i ) {
...
const deque<Studentas> kategorizuoti_studentas( deque<Studentas> &studentas ) {
    ...
    const deque<Studentas> vargselis( rem, studentas.end() );
