...
const vector<vector<Studentas>> kategorizuoti_studentas( vector<Studentas> &studentas ) {
    const auto rem = remove_if( studentas.begin(), studentas.end(),
                               []( const Studentas& studentas ) {
        return studentas.galutinis_paz < 5.0;
    } );
    
    const vector<Studentas> kietiekai( studentas.cbegin(), rem );
    const vector<Studentas> vargseliai( rem, studentas.cend() );
    
    return { kietiekai, vargseliai };
}
